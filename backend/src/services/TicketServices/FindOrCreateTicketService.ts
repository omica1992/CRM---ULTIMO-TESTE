...
import { createLock } from "../../helpers/LockHelper";
...
const FindOrCreateTicketService = async (
  ...
  // ✅ CORREÇÃO: Criar lock para evitar race condition
  const contactId = groupContact ? groupContact.id : contact.id;
  const lockKey = `ticket:create:${contactId}:${whatsapp.id}`;
  const release = await createLock(lockKey, 5000);
  ...
  try {
    ...
    // ✅ CORREÇÃO: Liberar o lock sempre, mesmo em caso de erro
    await release();
  }
};
